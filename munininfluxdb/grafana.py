...
class Panel:
    ...
    def sort_queries(self, order):
        ...
        self.queries.sort(key=lambda x: ordered_keys.index(
            x.field) if x.field in ordered_keys else len(self.queries))
    ...
    def process_graph_thresholds(self, fields):
        ...
        """
        warnings = {fields[field].settings.get(
            "warning") for field in fields if "warnings" in fields[field].settings}
        criticals = {fields[field].settings.get(
            "critical") for field in fields if "critical" in fields[field].settings}
    ...
    def process_graph_types(self, fields):
        ...
        """
        draw_list = [(field, fields[field].settings.get("draw", "LINE2"))
                     for field in fields]
        ...
        for field, draw in draw_list:
            ...
            if hasStack and draw.startswith("LINE") and not draw.startswith("LINESTACK"):
                ...
                if draw == 'LINE':
                    ...
                    # lines appear bigger on Grafana
        ...
        # colors
        self.alias_colors = {field: '#'+fields[field].settings.get(
            "colour") for field in fields if "colour" in fields[field].settings}
...
class Dashboard:
    ...
    def prompt_setup(self):
        ...
        setup['host'] = input(
            "  - host [http://localhost:3000]: ").strip() or "http://localhost:3000"
        setup['auth'] = None
        setup['filename'] = None

        while not GrafanaApi.test_host(setup['host']) and not setup['filename']:
            print("\n{0}We couldn't connect to {1}, please try again or leave empty to save to a local file{2}".format(
                Symbol.WARN_YELLOW, setup['host'], Color.CLEAR))
            setup['host'] = input("  - host: ").strip() or ""
            if not setup['host']:
                setup['filename'] = input(
                    "  - local file [/tmp/munin-grafana.json]: ").strip() or "/tmp/munin-grafana.json"
        ...
        if GrafanaApi.test_host(setup['host']):
            ...
            while setup['access'] not in ("proxy", "direct"):
                setup['access'] = input(
                    "  - data source access [proxy]/direct: ").strip() or "proxy"

        self.title = input("  Dashboard title [{0}]: ".format(
            self.title)).strip() or self.title
        graph_per_row = input(
            "  Number of graphs per row [2]: ").strip() or "2"
        setup['graph_per_row'] = int(graph_per_row)

        show_minmax = input(
            "  Show min/max/current in legend [y]/n: ").strip() or "y"
...
'''
    ...
    def upload(self):
        ...
        api.create_datasource(
            self.settings.influxdb['database'], self.settings.influxdb['database'])
    ...
    def generate_simple(title, structure):
        ...
        for series in structure:
            ...
            panel = row.add_panel(
                series['name'].split(".")[-1], series['name'])
...
class GrafanaApi:
    ...
    def create_datasource(self, name, dbname):
        ...
        r = requests.post(self.host + "/api/datasources",
                          json=body, auth=self.auth)
        return r.ok

    def create_dashboard(self, dashboardJson):
        r = requests.post(self.host + "/api/dashboards/db",
                          json={"dashboard": dashboardJson}, auth=self.auth)
        if r.ok:
